...
def takeImage():
	...
	while True:
		...
		cv2.namedWindow('Test picture') # 0 = resize not set to auto
		...
		if k%256 == 27:# ESC pressed
			...
			# SPACE pressed
			img_name = "opencv_frame_for_skin_detection.png"
			cv2.imwrite(img_name, image)
			print("Image written!")
			break
	print("Now we want to load the image")
	camera.release()
	cv2.destroyAllWindows()

def getSkinColour():
	global image_hsv
	image_src = cv2.imread("opencv_frame_for_skin_detection.png")
	if image_src is None:
		print("The image read is None.....")
		return
	cv2.namedWindow('bgr', 0) # 0 = resize not set to auto
	...
	cv2.putText(image_hsv,"Click on your skin on the HSV image!", (0,450), cv2.FONT_HERSHEY_SIMPLEX, 1, (200,255,155),2,cv2.LINE_AA)
...
def pick_color(event,x,y,flags,param):
	global image_hsv
	global lowerBoundSkin
	global upperBoundSkin
	if event == cv2.EVENT_LBUTTONDOWN:
	    ...
	    lowerBoundSkin = lower
	    upperBoundSkin = upper
...
def mainThing():
	...
	global lowerBoundSkin, upperBoundSkin
	...
	while(cap.isOpened()): 
		...
		if ret == True:
			...
			if initialFrameCounter == 0:
				...
				mask = cv2.inRange(hsv, np.array(lowerBoundSkin), np.array(upperBoundSkin))
